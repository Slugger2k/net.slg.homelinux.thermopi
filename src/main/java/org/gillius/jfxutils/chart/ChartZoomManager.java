...
/**
 * ChartZoomManager manages a zooming selection rectangle and the bounds of the
 * graph. It can be enabled via {@link #start()} and disabled via
 * {@link #stop()}. The normal usage is to create a StackPane with two children,
 * an XYChart type and a Rectangle. The Rectangle should start out invisible and
 * have mouseTransparent set to true. If it has a stroke, it should be of INSIDE
 * type to be pixel perfect.
 * <p/>
 * You can also use {@link JFXChartUtil#setupZooming(XYChart)} for a default
 * solution.
 * <p/>
 * Six types of zooming are supported. All are enabled by default. The drag
 * zooming can be disabled with the {@link #setMouseFilter} set to a mouse
 * filter that allows nothing. Mouse wheel zooming can be disabled via the
 ...
 * <li>Free-form zooming by the mouse wheel. The location of the cursor is taken
 * as the zoom focus point</li>
 ...
 * </ol>
 * <p/>
 * A lot of code in ChartZoomManager currently assumes there are no scale or
 * rotate transforms between the chartPane and the axes and plot area. However,
 * all translation transforms, layoutX/Y changes, padding, margin, and
 * setTranslate issues should be OK. This might be improved later, for example
 * JavaFX 8 is rumored to allow transform multiplication, which could solve
 * this.
 * <p/>
...
public class ChartZoomManager {
    /**
     * The default mouse filter for the {@link ChartZoomManager} filters events
    ...
    public static final EventHandler<MouseEvent> DEFAULT_FILTER = new EventHandler<MouseEvent>() {
        ...
        public void handle(MouseEvent mouseEvent) {
            // The ChartPanManager uses this reference, so if behavior changes,
            // copy to users first.
    ...
    /**
     * Construct a new ChartZoomManager. See {@link ChartZoomManager}
     ...
     * @param chart
     *            Chart to manage, where both X and Y axis are a
     *            {@link ValueAxis}.
     */
    public ChartZoomManager(Pane chartPane, Rectangle selectRect, XYChart<?, ?> chart) {
        ...
        handlerManager.addEventHandler(false, TouchEvent.TOUCH_RELEASED, new TouchEventZoomHandler(xAxis, yAxis));
    ...
    /**
     * Sets the mouse filter for starting the zoom action. If the filter
     * consumes the event with {@link Event#consume()}, then the event is
     * ignored. If the filter is null, all events are passed through. The
    ...
    /**
     * Start managing zoom management by adding event handlers and bindings as
     * appropriate.
     */
    public void start() {
    ...
    /**
     * Stop managing zoom management by removing all event handlers and
     * bindings, and hiding the rectangle.
     */
    public void stop() {
    ...
    private void onDragStart() {
        // Don't actually start the selecting process until it's officially a
        // drag
    ...
    private void onMouseReleased() {
        ...
        Rectangle2D zoomWindow = chartInfo.getDataCoordinates(selectRect.getTranslateX(), selectRect.getTranslateY(), rectX.get(), rectY.get());

        xAxis.setAutoRanging(false);
        yAxis.setAutoRanging(false);
        if (zoomAnimated.get()) {
            zoomAnimation.stop();
            zoomAnimation.getKeyFrames()
                    .setAll(new KeyFrame(Duration.ZERO, new KeyValue(xAxis.lowerBoundProperty(), xAxis.getLowerBound(), Interpolator.EASE_BOTH),
                            new KeyValue(xAxis.upperBoundProperty(), xAxis.getUpperBound(), Interpolator.EASE_BOTH),
                            new KeyValue(yAxis.lowerBoundProperty(), yAxis.getLowerBound(), Interpolator.EASE_BOTH),
                            new KeyValue(yAxis.upperBoundProperty(), yAxis.getUpperBound(), Interpolator.EASE_BOTH)),
                    new KeyFrame(Duration.millis(zoomDurationMillis.get()), new KeyValue(xAxis.lowerBoundProperty(), zoomWindow.getMinX(), Interpolator.EASE_BOTH),
                            new KeyValue(xAxis.upperBoundProperty(), zoomWindow.getMaxX(), Interpolator.EASE_BOTH),
                            new KeyValue(yAxis.lowerBoundProperty(), zoomWindow.getMinY(), Interpolator.EASE_BOTH),
                            new KeyValue(yAxis.upperBoundProperty(), zoomWindow.getMaxY(), Interpolator.EASE_BOTH)));
            zoomAnimation.play();
        } else {
            zoomAnimation.stop();
        ...
        public void handle(ScrollEvent event) {
            ...
            if (eventType == ScrollEvent.SCROLL_STARTED) {
                ...
                ignoring = false;
                    ...
                    !ignoring &&
                    // inertia from non-wheel gestures might have touch count of
                    // 0
            !event.isInertia() &&
                ...
                // If we are are doing a zoom animation, stop it. Also of note
                // is that we don't zoom the
                // mouse wheel zooming. Because the mouse wheel can "fly" and
                // generate a lot of events,
                // animation doesn't work well. Plus, as the mouse wheel changes
                // the view a small amount in
                // a predictable way, it "looks like" an animation when you roll
                // it.
                // We might experiment with mouse wheel zoom animation in the
                // future, though.
                zoomAnimation.stop();

                // If we wheel zoom on either axis, we restrict zooming to that
                // axis only, else if anywhere
                ...
                // At this point we are a mouse wheel event, based on everything
                // I've read
                Point2D dataCoords = chartInfo.getDataCoordinates(eventX, eventY);

                // Determine the proportion of change to the lower and upper
                // bounds based on how far the
                // cursor is along the axis.
                double xZoomBalance = getBalance(dataCoords.getX(), xAxis.getLowerBound(), xAxis.getUpperBound());
                double yZoomBalance = getBalance(dataCoords.getY(), yAxis.getLowerBound(), yAxis.getUpperBound());

                // Are we zooming in or out, based on the direction of the roll
                double direction = -Math.signum(event.getDeltaY());

                // TODO: Do we need to handle "continuous" scroll wheels that
                // don't work based on ticks?
                // If so, the 0.2 needs to be modified
                double zoomAmount = 0.25 * direction;
                ...
                if (zoomMode == ZoomMode.Horizontal) {
                    ...
                    zoomAnimation.getKeyFrames()
                            .setAll(new KeyFrame(Duration.ZERO, new KeyValue(xAxis.lowerBoundProperty(), xAxis.getLowerBound()),
                                    new KeyValue(xAxis.upperBoundProperty(), xAxis.getUpperBound())),
                            new KeyFrame(Duration.millis(250), new KeyValue(xAxis.lowerBoundProperty(), xAxis.getLowerBound() - xZoomDelta * xZoomBalance),
                                    new KeyValue(xAxis.upperBoundProperty(), xAxis.getUpperBound() + xZoomDelta * (1 - xZoomBalance))));
                    zoomAnimation.play();
                }
                ...
                if (zoomMode == ZoomMode.Vertical) {
                    ...
                    zoomAnimation.getKeyFrames()
                            .setAll(new KeyFrame(Duration.ZERO, new KeyValue(yAxis.lowerBoundProperty(), yAxis.getLowerBound()),
                                    new KeyValue(yAxis.upperBoundProperty(), yAxis.getUpperBound())),
                            new KeyFrame(Duration.millis(250), new KeyValue(yAxis.lowerBoundProperty(), yAxis.getLowerBound() - yZoomDelta * yZoomBalance),
                                    new KeyValue(yAxis.upperBoundProperty(), yAxis.getUpperBound() + yZoomDelta * (1 - yZoomBalance))));

                    zoomAnimation.play();
                }

                if (zoomMode == ZoomMode.Both) {
                    yAxis.setAutoRanging(false);
                    xAxis.setAutoRanging(false);

                    zoomAnimation.getKeyFrames()
                            .setAll(new KeyFrame(Duration.ZERO, new KeyValue(yAxis.lowerBoundProperty(), yAxis.getLowerBound(), Interpolator.EASE_BOTH),
                                    new KeyValue(yAxis.upperBoundProperty(), yAxis.getUpperBound(), Interpolator.EASE_BOTH)),
                            new KeyFrame(Duration.millis(250), new KeyValue(yAxis.lowerBoundProperty(), yAxis.getLowerBound() - yZoomDelta * yZoomBalance, Interpolator.EASE_BOTH),
                                    new KeyValue(yAxis.upperBoundProperty(), yAxis.getUpperBound() + yZoomDelta * (1 - yZoomBalance))),
                            new KeyFrame(Duration.ZERO, new KeyValue(xAxis.lowerBoundProperty(), xAxis.getLowerBound(), Interpolator.EASE_BOTH),
                                    new KeyValue(xAxis.upperBoundProperty(), xAxis.getUpperBound(), Interpolator.EASE_BOTH)),
                            new KeyFrame(Duration.millis(250), new KeyValue(xAxis.lowerBoundProperty(), xAxis.getLowerBound() - xZoomDelta * xZoomBalance, Interpolator.EASE_BOTH),
                                    new KeyValue(xAxis.upperBoundProperty(), xAxis.getUpperBound() + xZoomDelta * (1 - xZoomBalance), Interpolator.EASE_BOTH)));
